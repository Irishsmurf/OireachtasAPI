...
------------- | ------------- | ------------- | -------------
 **bill_status** | **list[str]**| An array which is used to filter legislation by status detailed in default settings below.  Separated by comma. | [optional] [default to [Current, Withdrawn, Enacted, Rejected, Defeated, Lapsed]]
